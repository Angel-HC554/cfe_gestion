<x-layouts.app>
    <div class="flex h-full flex-1 flex-col gap-4 mx-10">
        <div class="flex items-center justify-between">
            <flux:heading size="xl">Supervisiónes Semanales</flux:heading>
            <h3 class="font-semibold">Mostrando: {{ strtoupper($nombreMes) }}</h3>
        </div>
    <form method="GET" action="{{ route('supervicion_semanal.index') }}" class="filtros-panel">
...
<div id="contenido-tabla"style="display:none;" class="tabla-scrollable"> <table class="tabla-matriz">
    ...
    <tr>
        ...
        @foreach($semanasDelMes as $i => $semana)
            <th>
                Semana {{ $i + 1 }}<br>
                <small>{{ $semana['inicio']->format('d M') }} - {{ $semana['fin']->format('d M') }}</small>
            </th>
        @endforeach
    </tr>
</thead>
